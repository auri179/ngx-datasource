...
@Component({
  selector: "tc-datasource",
  template: `
    <ng-content></ng-content>
  `
})
export class DataSourceComponent implements OnChanges, DoCheck {
  ...
  @Output()
  public sortByChange = new EventEmitter<string | string[]>();
  @Output()
  public sortOrderChange = new EventEmitter<string>();

  private mustRecalculateData = false;
  public data: any[];
  //public rowCount = this.data.length;
  ...
  public setPage(activePage: number, rowsOnPage: number): void {
    if (this.rowsOnPage !== rowsOnPage || this.activePage !== activePage) {
      ...
      this.onPageChange.emit({
        activePage: this.activePage,
        rowsOnPage: this.rowsOnPage,
        dataLength: this.datasource ? this.datasource.length : 0
      });
    }
  }
  ...
  private recalculatePage() {
    const lastPage = Math.ceil(this.datasource.length / this.rowsOnPage);
  ...
  public ngOnChanges(changes: { [key: string]: SimpleChange }): any {
    ...
    if (changes["sortBy"] || changes["sortOrder"]) {
      if (!_.includes(["asc", "desc"], this.sortOrder)) {
        console.warn(
          "angular2-datatable: value for input mfSortOrder must be one of ['asc', 'desc'], but is:",
          this.sortOrder
        );
        this.sortOrder = "asc";
      }
    ...
    if (changes["inputData"]) {
      this.datasource = changes["inputData"].currentValue || [];
  ...
  public ngDoCheck(): any {
    const changes = this.diff.diff(this.datasource);
    if (changes) {
      this.recalculatePage();
      this.mustRecalculateData = true;
    }
